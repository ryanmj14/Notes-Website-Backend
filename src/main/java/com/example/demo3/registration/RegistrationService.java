...
import com.example.demo3.email.EmailSender;
import com.example.demo3.registration.token.ConfirmationToken;
import com.example.demo3.registration.token.ConfirmationTokenService;
...
public class RegistrationService {
    ...
    public String confirmToken(String token) {
        ...
        if (confirmationToken.getConfirmedAt() != null) {
            throw new IllegalStateException("This email address has already been confirmed.");
        }
    ...
    public String isEmailVerified(String email) {
    	boolean isVerified = false;
    	Optional<AppUser> thisUser = appUserRepository.findByEmail(email);
    	if(thisUser.isPresent()) {
    		isVerified = thisUser.get().isEnabled();
    	} else {
    		throw new IllegalStateException(thisUser.get().getEmail());
    	}
